...
def train(model, optimizer, dataloader, epochs, device, pad_tag):
    ...
    batch_loss = partial(get_loss, encoder = encoder, decoder = decoder,
                         tag_classifier = tag_classifier, device = device,
                         pad_tag = pad_tag)
    running_loss = None
    ...
    for epoch in range(epochs):
        ...
        for batch in batches:
            ...
            batches.set_postfix_str(
                "Total Running Loss: {:.2f}, Tag Loss: {:.2f}, Decoder Loss: {:.2f}" \
                .format(running_loss, detached_tag_loss, detached_decoder_loss)
                )
