...
def conv_ngram_encoder_factory(input_dim: int, ngram: int,
...
class Char2TokenEncoder(nn.Module):
    ...
    def __init__(self, vocabulary_size, embedding_dim, hidden_dim,
        ...
        ngram_encoders = [
            conv_ngram_encoder_factory(embedding_dim, n,
...
class Encoder(nn.Module):
    ...
    """
    def __init__(self, vocabulary_size, embedding_dim, hidden_dim, max_ngram,
                 dropout = 0.0):
        super().__init__()
        ...
        self.char2token_encoder = Char2TokenEncoder(
            vocabulary_size, embedding_dim, hidden_dim,
            max_ngram, dropout = dropout
            )
    ...
    def forward(self, inputs):
        ...
        token_embeddings, char_embeddings, sent_lengths, token_lengths = \
            self.char2token_encoder(inputs)

        # Calculate contextual word embeddings
        contextual_embeddings = self.contextual_encoder(
            token_embeddings, sent_lengths
            )
