...
if __name__ == '__main__':
    ...
    train_data_indexed, train_data, discarded = \
        index_dataset(train_data, char2index, tag_encoder)
    print(f"Discarded {discarded} sentences from train data")
    print("Index eval data")
    eval_data_indexed, eval_data, discarded = \
        index_dataset(eval_data, char2index, tag_encoder, eval=True)
    ...
    train_dataloader = DataLoader(train_data_indexed,
                                  batch_size = batch_size,
                                  collate_fn = collate_fn,
                                  shuffle = True)
    eval_dataloader = DataLoader(eval_data_indexed,
                                 batch_size = batch_size,
    ...
    model, optimizer = train(model, optimizer, train_dataloader, epochs,
                             device, pad_tag)
