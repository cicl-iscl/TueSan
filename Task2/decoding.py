...
def informed_decoding(decoder, encoder_output, char_embeddings, token_lengths,
                      allowed_stems, char2index):
    ...
    max_len = safe_max([safe_max([len(stem) for stem in stems]) for stems in allowed_stems])
    ...
    for step in range(max_len):
        ...
        output, hidden = decoder._forward_step(
            last_output, decoder_input, char_embeddings,
            token_lengths, hidden=hidden
            )
            ...
            # character
                ...
                # according to decoder
                    ...
                    updated_allowed_stems = [
                        stem for stem in current_allowed_stems
...
def greedy_decoding(decoder, encoder_output, char_embeddings, token_lengths,
    ...
    for _ in range(max_len):
        output, hidden = decoder._forward_step(last_output, decoder_input,
                                               char_embeddings, token_lengths,
                                               hidden=hidden)
