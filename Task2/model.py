...
def build_model(config, vocabulary, tag_encoder):
    ...
    # Encoder
    encoder = Encoder(len(vocabulary), embedding_size, encoder_hidden_size,
                      encoder_max_ngram, dropout = dropout)
    
    # Decoder (for predicting stems)
    decoder = RecurrentDecoder(len(vocabulary), embedding_size,
                               encoder.output_size, decoder_hidden_dim,
                               decoder_num_layers, dropout = dropout)
    
    # Tag classifier -> morphological tags
    tag_classifier = mlp_classifier_factory(
        encoder.output_size, classifier_hidden_dim, len(tag_encoder.classes_),
        layers = classifer_num_layers, dropout = dropout
        )
